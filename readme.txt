...
05-06 강사님 주신 내용합친 후 실험완료 결론: 패킷 캡처에서 프러미스큐어스 모드 실행을 위해선
루트 권한이 필요. 그러나 루트권한상태에선 보안상이유로 브라우저 실행이 불가함. 이에따라
브라우저실행은 포기하고 악성사이트 진입시 브라우저 끄는것만진행해야할 듯 싶음. 
1차 필터링은 강사님 방향대로 http get메세지에서  도메인을 얻어내어 비교 후 필터링
2차 필터링은 도메인을 얻을 수 없는 패킷중에서 syn 패킷만 골라낸 후
(syn인 이유: 웹연결을 위해서는 3way가 반드시 실행되어야하므로 syn패킷은 발생할 수 밖에없고
 평균적으로 발생횟수가 가장 적음)
 얻어낸 ip를 기존 진행대로 db에 있는지 검사 후 api쿼리 진행 후 db에 추가.
 여기서 개선할 점은 api검사후 유해한 ip만 추가하는것이아니라 검사가 끝낸 ip는 모두 추가하여
 무해하다고 판단된 ip도 db에 추가하여 이후에 추가api호출이 없도록 개선할 여지 있음.

 05-08 네트워크 dev 두개를 동시에 진행 하기 위해 'lo'와 "enp0s3"두개의 핸들을 만들어야해서 
 멀티스레드를 진행함. 호스트주소를 보고 필터링하는것은 루프백패킷이므로 lo 에 들어가고 밖으로 나가는
 패킷은 enp0s3 에 들어감.
