...
void sendraw(const u_char* pre_packet, int mode)
{
		...
		source_address.s_addr = ((struct iphdr *)(pre_packet + 14))->daddr ;    // twist s and d address
		...
		post_payload_size = strlen(packet_data) + 40;   // Content-Length: header is changed so post_payload_size is increased.
		...
		memcpy ( (char*)packet + 40, packet_data, post_payload_size) ;
